...
def isNumber(numIn):
  try:
    ...
    return False   
#  return isinstance(numIn,(int,float,complex))
...
def getScaleNum(strNum):
  import pandas as pd; from re import search
  if isinstance(strNum,pd.Series): return strNum.apply(getScaleNum) 
...
def numToSi(floatNum,**kargs):
  import si_prefix, pandas as pd, math, re
  if isinstance(floatNum,pd.Series): return floatNum.apply(numToSi,**kargs)
  si_prefix.SI_PREFIX_UNITS = 'yzafpnum kMGTPEZY'
  if not isNumber(floatNum): return floatNum
  elif math.isnan(float(floatNum)): return 'NaN'
  else: 
    test = si_prefix.si_format(floatNum,**kargs);
    if re.search(r'\.',test): test = re.sub(r'(\d+)\.0+ |(\d+\.[1-9]+)0+ ',r'\1\2',test)
    return test.replace(' ','')
