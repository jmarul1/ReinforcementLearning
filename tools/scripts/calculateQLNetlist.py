...
argparser = argparse.ArgumentParser(description='This program calculates the inductance of an inductor subcircuit file.')
...
argparser.add_argument('-plot', dest='plotme', nargs='*', choices=['Q','L','R','all'], help='displays a plot picture for the sparameter.')
...
#if not os.getenv('whoami') == 'jmarulan': 
...
for iiCktFile in args.cktFiles:
  ...
  if args.sim == 'scs':    
    freq,Q['diff'],L['diff'],Q['se'],L['se'],R['diff'],R['se'] = cktParam.getQPR(firstFreq=fFreq,stepFreq=sFreq,lastFreq=mFreq,device='ind')#,tmpTarget='.')
  else:
    freq,Q['diff'],L['diff'],R['diff'] = cktParam.getQPR_AC(firstFreq=fFreq,stepFreq=sFreq,lastFreq=mFreq,device='ind',sim='hsp',rEnd=1e9)#,tmpTarget='.')
    freq,Q['se'],L['se'],R['se'] = cktParam.getQPR_AC(firstFreq=fFreq,stepFreq=sFreq,lastFreq=mFreq,device='ind',sim='hsp',rEnd=1e-6)
  ...
  if args.mode: #if single or diff specified
    ...
    for ii in range(len(freq)):
      results += ','.join([str(freq[ii]),str(Q[args.mode][ii]),str(L[args.mode][ii]),str(R[args.mode][ii])]) + '\n'
  else:
    results = 'Freq(GHz),Qdiff,Ldiff(nH),Qse,Lse(nH),Rdiff(Ohms),Rse(Ohms)\n'
    for ii in range(len(freq)):
      results += ','.join([str(freq[ii]),str(Q['diff'][ii]),str(L['diff'][ii]),str(Q['se'][ii]),str(L['se'][ii]),str(R['diff'][ii]),str(R['se'][ii])]) + '\n'
  ...
  if args.csvFile != None:
    ...
    with open(prefix+cktFileName+'_QL.csv','w') as fout:
      fout.write(results)
  else: print(results)
  ...
  if type(args.plotme) == list:
    ...
    if any(args.plotme):
      if args.plotme[0] == 'all': plotArgs = {'Q':Q,'L':L,'R':R} #if all is asked for
      else:
        plotArgs = {}
