...
def convertToSp(path,outName,ext):
  ...
  if sb.call('/p/fdk/gwa/jmarulan/utils/scripts/citi2ts '+path,shell=True) == 0:
    srcName = os.path.basename(os.path.splitext(path)[0])+ext; outName += ext
    if not(os.getcwd()==os.path.dirname(path) and srcName==outName):
...
def calcRange(argsIn):
  ...
  for ii in xrange(len(citiObj.VAR)-1):
  ...
  return range(offset,offset+citiObj.VAR[-1][2]),suffix  
...
if any(map(lambda ii: re.search('-h|--help',ii),sys.argv[1:])) or len(sys.argv[1:]) == 0: tempParser.print_help()
