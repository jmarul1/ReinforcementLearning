...
arguments = ' '.join(args.cellNames)+' -lib '+args.libName+' -outdir '+args.outDir+' -viewName '
argumentsAdd = (' -layermap '+args.lmap+' ' if args.lmap else '')+(' -objectmap '+args.omap+' ' if args.omap else '')
subprocess.call('strmOut.py '+arguments+args.viewNames[0]+argumentsAdd,shell=True)
subprocess.call('strmCdl.py '+arguments+args.viewNames[1],shell=True)
