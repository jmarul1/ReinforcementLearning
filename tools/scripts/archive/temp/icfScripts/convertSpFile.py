...
for nextLine in fDataLns:
  ...
  ## fix the subcircuit line
    ...
    if terminals == 4: terminalsStr = "ct1_ehv ct2_ehv n_ehv p_ehv"
    elif terminals == 3: terminalsStr = "ct_ehv n_ehv p_ehv"
    else: terminalsStr = "n_ehv p_ehv"
    subLnMatch = re.search(r"^[ \t]*(\.?\w+)[ \t]+(\w+)[ \t]+",nextLine)
    outLine = subLnMatch.group(1)+" "+subLnMatch.group(2)+" "+terminalsStr+"\n"
  ## fix the netlist
  else:
    outLine = re.sub(r"\bn0\b","0",nextLine)
    outLine = re.sub(r"\bn1\b","n_ehv",outLine)
    outLine = re.sub(r"\bn2\b","p_ehv",outLine)
    if terminals == 3:
