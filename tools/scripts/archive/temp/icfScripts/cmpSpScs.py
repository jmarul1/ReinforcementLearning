...
argparser.add_argument('-cktfile', dest='cktFile', help='if given, this file is used for ckt comparison')
...
fileBase = re.sub('(_lowQ|_typQ|_highQ)$','',fileName,flags=re.I)

## if requested graph everything otherwise choose the skew
if args.cmpAll: skewList = ['lowQ','typQ','highQ']
else: 
  test = re.search('(lowQ|typQ|highQ)$',fileName,flags=re.I)
  if test: skewList = [test.group(1)]
  else: raise IOError('Wrong format for the name of the file: '+args.spFile)

## graph colors
pColor = dict(lowQ='r',typQ='k',highQ='b')
...
for iiSkew in skewList:  
  ...
  cktParam = ckt.read(args.cktFile) if args.cktFile else ckt.read(fileBase+'_'+iiSkew+args.cktext)
  ## Set freqs and get values
  mFreq = sparam.freq[-1]*sparam.freqM; fFreq = sparam.freq[0]*sparam.freqM; sFreq = (mFreq-fFreq)/(len(sparam.freq)-1)
  if args.sim == 'hsp':
  ...
  for iiP,var in enumerate(args.plot):
    ...
    Layouts[iiP].plot(sparam.freq,spPlotData[iiP],'o'+pColor[iiSkew],fillstyle='none')
