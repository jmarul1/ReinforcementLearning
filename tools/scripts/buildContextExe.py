...
def writeSkillCmdFile(context, filelist):
    ...
    try:
        ...
        print('setContext.autoload = \"skillDev.cxt\";', file=fh);
        print('saveContext.autoload = \"skillDev.cxt\";', file=fh);
        print('let( ()', file=fh)
        print('    (sstatus writeProtect t)', file=fh) 
        print("    setContext(\"%s\")"%(context), file=fh)
        for f in filelist:
            print("    println(\"%s\")"%("Loading " + f), file=fh)
            print("    unless( errset( load(\"%s\") t) exit(1))"%(f), file=fh)
        print("    unless( errset(saveContext(\"%s\") t) exit(1))"%(contextfile), file=fh)
        print('    (sstatus writeProtect nil)', file=fh) 
        print(")", file=fh) 
