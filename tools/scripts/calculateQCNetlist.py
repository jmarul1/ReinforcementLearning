...
for iiCktFile in args.cktFiles:
  ...
  if args.sim == 'scs':  #spectre
    if args.AC: #AC analysis
      ...
      if args.mode == 'mim': freq,params = cktParam.getQPR(firstFreq=fFreq,stepFreq=sFreq,lastFreq=mFreq,device='mim')
  ...
  if args.mode: #if single or diff specified
    if args.mode in ['diff','se']:
      ...
      for ii in range(len(freq)):
        results += str(freq[ii])+','+str(Q['diff'][ii])+','+str(C['diff'][ii])+','+str(R['diff'][ii])+','
	results += (','.join(str(params[kk][ii]) for kk in params.keys()))+'\n'
  else:
