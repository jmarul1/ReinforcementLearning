...
def checkGds(path):
  if os.path.isfile(path):
    if os.path.splitext(path)[1] in ['.gds','.oas']: return os.path.realpath(path)
    else: return False
  else: raise argparse.ArgumentTypeError('File doesn\'t exist or is a directory: '+path)

def getProject(): import os; return os.getenv('PROCESS_NAME') 

def getLvlLayer(): 
  import subprocess
  test = subprocess.run("find $ISSRUNSETS/ -name '*dx_icv_lvl_assign.rs'",shell=True,stdout=subprocess.PIPE)
  layer = test.stdout.decode()
...
argparser.add_argument('-lf', dest='lf', default = getLvlLayer(), help='layer file, defaults to current $ISSRUNSET')
...
gds1,gds2 = args.gds
cell1,cell2 = [os.path.basename(os.path.splitext(ff)[0]) for ff in args.gds]
...
if args.tool == 'icv_lvl':
  ...
  cpus = '-dp10'
  cmd = ' '.join(['icv_lvl',gds1,gds2,cpus,'-lf',args.lf,'-c1',cell1,'-c2',cell2,'-text']) 
